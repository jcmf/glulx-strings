        ...
        case 'OBJ':
          while (!block.eof()) {
            ...
            switch (type) {
              ...
              case 2:
                (function() {
                  var addr, b, bad, buf, byte, chars, data_end, data_len, data_start, len, pos, _i, _len, _results;
                  block.skip(2);
                  len = block.u16();
                  addr = block.tell();
                  buf = decrypt(block.bytes(len));
                  _results = [];
                  for (pos = _i = 0, _len = buf.length; _i < _len; pos = ++_i) {
                    byte = buf[pos];
                    ...
                    chars = (function() {
                      ...
                      _ref = buf.slice(data_start, data_end);
                      _results1 = [];
                    ...
                    _results.push(cb(chars.join(''), addr + data_start));
                  }
                  return _results;
                })();
                break;
